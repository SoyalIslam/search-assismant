...
def generate_js():
    if not os.path.exists(RESULTS_PATH):
        print(f"❌ Error: {RESULTS_PATH} not found. Please run verify_accuracy.py first.")
        return
    if not os.path.exists(PATTERNS_PATH):
        print(f"❌ Error: {PATTERNS_PATH} not found. Please run analyze_patterns.py first.")
        return
    ...
    # JavaScript code template with embedded data
    js_content = f"""// Auto-generated Dashboard Assets and Application Logic
const RESEARCH_DATA = {json.dumps(results_data, indent=2)};
const PATTERNS_DATA = {json.dumps(patterns_data, indent=2)};
...
"""
    ...
    print(f"✅ Generated {APP_JS_PATH} with embedded data.")
